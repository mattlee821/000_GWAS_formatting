...
# marginal meta ====
# joint ====
file_list <- list.files(paste0("/data/GWAS_data/files/", DATA, pattern = ".gz", full.names = T))
for (i in 1:length(file_list)){
  data <- fread(file_list[i], header = T)
  data$chr_pos <- gsub("\\_.*","",data$MarkerName) # change MarkerName to you col with chr:pos
