...
for ancestry in "${ancestries[@]}"; do
  # Define the processed directory for the current ancestry
  DIRECTORY_PROCESSED="/data/GWAS_data/files/sun_2023_PMID37794186/processed/"
  DIRECTORY_ANCESTRY="${ancestry}/" # Assuming ancestry folder name matches ancestry variable

  # Navigate to the correct ancestry directory
  cd "${DIRECTORY_PROCESSED}${DIRECTORY_ANCESTRY}"

  # Define the process_tar_file function
  process_tar_file() {
    ...
    # Move the concatenated file to the processed directory
    ...
    gzip -f "${DIRECTORY_PROCESSED}${DIRECTORY_ANCESTRY}${FILENAME}"
  ...
  parallel -j 1 process_tar_file ::: $(find "${DIRECTORY_PROCESSED}${DIRECTORY_ANCESTRY}" -type f -name "*.tar" | sort -r)
