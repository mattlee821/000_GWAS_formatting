...
#SBATCH --job-name=window-1mb-UKB-combined
#SBATCH --nodes=1
...
OUTPUT_FOLDER="/data/GWAS_data/work/UKB_PPP/cis-snps/combined/1mb"
...
while IFS=$'\t' read -r FILE_PATH CHR POS FILE_OUT; do
    ...
    zcat "$FILE_PATH" | awk -v chr="$CHR" -v pos="$POS" '$1 == chr && ($20 >= (pos - 1000000) && $20 < (pos + 1000000)) { print $0 }' > "${FILE_OUT}"
    
    echo "Line count for $FILE_OUT: $(wc -l < "${FILE_OUT}")"

    # Move the created file to the 1mb folder
